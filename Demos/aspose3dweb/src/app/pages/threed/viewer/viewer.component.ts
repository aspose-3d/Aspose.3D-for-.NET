...
export class ViewerComponent implements OnInit {
  ...
  public fnName: string = "viewer";
  public threedViewerTitle: string;
  public threedViewerTitleSub: string;
  ...
  public application: string = "viewer";
  ...
  ngOnInit(): void {
    ...
    var m = locationUrl.match(/viewer\/(\w+)/);
    if (m) {
      this.Suffix = m[1].toUpperCase();
      this.threedViewerTitle = this.resourceENService.get("ThreedFreeOnline") + this.Suffix + this.resourceENService.get("ThreedViewerPrinting");
      this.threedViewerTitleSub = this.resourceENService.get("ThreedViewerViewing") + this.Suffix + this.resourceENService.get("ThreedViewerDevice");
      this.acceptStr = this.resourceENService.get(this.productName + this.Suffix + "ValidationExpression");
    } else {
      this.threedViewerTitle = this.resourceENService.get("threedViewerTitle");
      this.threedViewerTitleSub = this.resourceENService.get("threedViewerTitleSub");
  ...
  async viewfile() {
    ...
    try {
      ...
      let uploadResult: any = await this.fileService.upload('api/viewer/upload', this.filelist);
      if (uploadResult.data.state == "Success") {
        this.sessionId = uploadResult.data.sessionId;
        this.route.navigate(['viewer'], { queryParams: { session: this.sessionId } });
        this.firstshow = false;
        this.downshow = true;
      } else {
        this.sessionId = uploadResult.data.sessionId;
