...
namespace Aspose.ThreeD.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeThreeDContext.atcc = new AsposeThreeDContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeThreeDContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeThreeDContext.Current.Cache.Add(
				   SessionID,
					resources,
